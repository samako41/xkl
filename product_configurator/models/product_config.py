...
def flatten(list_of_lists):
    """Custom flatten to replace odoo.tools.misc.flatten in Odoo 18."""
    return list(chain.from_iterable(
        item if isinstance(item, (list, tuple, set)) else [item]
...
class ProductConfigSession(models.Model):
    ...
    def get_cfg_weight(self, value_ids=None, custom_vals=None):
        """Computes the weight of the configured product based on the configuration passed in."""
        ...
        weight_extra = sum(product_tmpl_attr_val.weight_extra for product_tmpl_attr_val in product_tmpl_attr_values)
    ...
    def get_cfg_price(self, value_ids=None, custom_vals=None):
        """Computes the price of the configured product based on selected attributes."""
        if value_ids is None:
            value_ids = self.value_ids.ids
        if custom_vals is None:
            custom_vals = {}
    ...
    def flatten_val_ids(self, value_ids):
        """Safely flatten a list that may contain integers and/or nested iterables."""
        flat_val_ids = []
        for item in value_ids:
            if isinstance(item, (list, tuple, set)):
                flat_val_ids.extend(item)
            else:
                flat_val_ids.append(item)
        return list(set(flat_val_ids))  # Unique list of value_ids
    ...
    def get_onchange_specifications(self, model):
        ...
        one2many or many2many there is view-reference(using : list_view_ref)
