...
class ProductConfigSession(models.Model):
    ...
    def create_get_bom(self, variant, product_tmpl_id=None, values=None):
        ...
        # Check for existing specific BoM
        existing_bom = mrp_bom_model.search([
        ...
        # Check for parent BoM
        parent_bom = mrp_bom_model.search([
            ("product_tmpl_id", "=", product_tmpl_id.id),
            ("product_id", "=", False),
        ...
        if not parent_bom:
            # No parent BoM: create lines from attribute products
            ...
            # Use parent BoM lines and filter by attribute values
        ...
        # Create the BoM if we have lines
        if bom_lines:
            bom_values = {
            ...
            new_bom = mrp_bom_model.create(bom_values)

            # Copy operations from parent BoM if exists
            if new_bom and parent_bom:
                for operation_line in parent_bom.operation_ids:
                    operation_line.copy(default={"bom_id": new_bom.id})
